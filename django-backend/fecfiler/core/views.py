...
@api_view(["POST"])
def create_f3x_inkind_bitcoin_json_file(request):
    #creating a JSON file so that it is handy for all the public API's   
    try:
        report_id = request.POST.get('report_id')
        #import ipdb;ipdb.set_trace()
        ...
        if comm_info:
            ...
            for f3_i in f_3x_list:
                ...
                for entity_obj in entity_id_list:
                    ...
                    entity_id_child_list = get_entity_id(f3_i['report_id'], f3_i['cmte_id'], entity_obj['transaction_id'])
                    ...
                    for entity_child_obj in entity_id_child_list:
                        ...
                        response_dict_out['expenditureDate'] = entity_child_obj['contribution_date'].replace('-','')
                        response_dict_out['expenditurePurposeDescription'] = entity_child_obj['purpose_description']
