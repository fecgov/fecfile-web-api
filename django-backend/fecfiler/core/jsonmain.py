...
def create_json_builders(request):
    try:
        ...
        for field in MANDATORY_INPUTS:
            if field not in request.data or request.data.get(field) in [
                None,
                "null",
                "",
                "",
        ...
        if form_type in ["F3X", "F24", "F3L"]:
            ...
            ALL_CHILD_TRANSACTION_TYPES_LIST = get_all_child_transaction_identifers(
                form_type
            )
            ...
            if form_type == "F3X" and (not transaction_flag):
                output["data"]["summary"] = get_f3x_summary_details(report_id, cmte_id)
            if form_type == "F24":
                output["data"]["reportPrint"] = False if transaction_flag else True
            if form_type == "F3L" and (not transaction_flag):
                output["data"]["summary"] = get_f3l_summary_details(report_id, cmte_id)
            output["data"]["schedules"] = {}
            for schedule_name in schedule_name_list:
                schedule = SCHED_SCHED_CODES_DICT.get(schedule_name.get("sched_type"))
                if schedule:
                    ...
                    for identifier in list_identifier:
                        ...
                        for transaction in parent_transactions:
                            ...
                            if (
                                transaction.get("lineNumber")
                                not in EXCLUDED_LINE_NUMBERS_FROM_JSON_LIST
                            ):
                                if (
                                    ...
                                    output["data"]["schedules"]["SL-A"].append(
                                        transaction
                                    )
                                elif (
                                    transaction.get("transactionTypeIdentifier")
                                    ...
                                    output["data"]["schedules"]["SL-B"].append(
                                        transaction
                                    )
                                else:
        ...
        if call_from == "PrintPreviewPDF":
            ...
            file_obj = {
                "fecDataFile": ("data.json", open(tmp_path, "rb"), "application/json")
            }
            add_log(
                report_id,
                cmte_id,
                ...
                if call_from == "Submit":
                    ...
                    if submission_response.json()["result"][0]["status"] == "ACCEPTED":
                        ...
                        return submit_report(
                            request, submission_id, beginning_image_number, fec_id
                        )
                    return JsonResponse(
                        submission_response.json(), status=status.HTTP_201_CREATED
                    )
                else:
                    return JsonResponse(resp.json(), status=status.HTTP_201_CREATED)
    except Exception as e:
        return Response(
            "The create_json_builders is throwing an error: " + str(e),
