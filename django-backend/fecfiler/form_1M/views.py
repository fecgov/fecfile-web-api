...
def noneCheckMissingParameters(
    parameter_name_list, checking_dict=None, value_dict=None, function_name="blank"
):
    try:
        missing_list = []
        none_list = []
        ...
        for key, value in request_dict.items():
            if key in DICT_F1M_COLUMNS_MAP_INPUT:
                if DICT_F1M_COLUMNS_MAP_INPUT[key] in db_key_list:
                    output_dict[DICT_F1M_COLUMNS_MAP_INPUT[key]] = (
                        value if value else None
                    )
        logger.debug(output_dict)
        return output_dict
    except Exception as e:
        raise Exception("The f1m_sql_dict function is throwing an error: " + str(e))
...
def post_sql(request_dict, table, error_function):
    try:
        ...
        noneCheckMissingParameters(
            ["report_id", "cmte_id"],
            checking_dict=request_dict,
            value_dict=request_dict,
            function_name=table + "-POST",
        )
        value_list = []
        key_string = ""
        param_string = ""
        ...
        with connection.cursor() as cursor:
            sql = """INSERT INTO public.{}({}) VALUES ({})""".format(
                table, key_string[:-2], param_string[:-2]
            )
...
def put_sql(request_dict, table, error_function="blank"):
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if cursor.rowcount == 0:
                raise Exception(
                    """Failed to update data in {} table. report Id: {},
                    committee Id: {}""".format(
                        table, request_dict["report_id"], request_dict["cmte_id"]
                    )
                )
    except Exception as e:
        raise Exception(
    ...
    try:
        with connection.cursor() as cursor:
            ...
            sql = """UPDATE public.{} SET delete_ind = %s, last_update_date=%s
                WHERE cmte_id=%s AND report_id=%s""".format(
                table
            )
            cursor.execute(
                sql,
                [
                    request_dict["delete_ind"],
                    datetime.datetime.now(),
                    request_dict["cmte_id"],
                    request_dict["report_id"],
                ],
            )
            ...
            if cursor.rowcount == 0:
                raise Exception(
                    """Failed to {} data in {} table. report Id: {},
                    committee Id: {}""".format(
                        print_text,
                        table,
                        request_dict["report_id"],
...
def remove_sql(request_dict, table, error_function="blank"):
    try:
        with connection.cursor() as cursor:
            ...
            if cursor.rowcount == 0:
                raise Exception(
                    """Failed to remove report from {} table. report Id: {},
                    committee Id: {}""".format(
                        table, request_dict["report_id"], request_dict["cmte_id"]
                    )
                )
...
def check_candidate_number(candidate_number):
    try:
        if 1 <= int(candidate_number) <= 5:
            ...
            raise Exception(
                """The candidate number can only be in between and including
                1 and 5. Input received: {}""".format(
                    candidate_number
                )
            )
...
def committee_master_get(request_dict):
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                """SELECT json_agg(t) FROM ({}) AS t""".format(sql),
                [request_dict["cmte_id"]],
            )
            ...
            if output_dict:
                ...
                raise Exception(
                    """The committee Id: {} does not exist in
                    committee_master table.""".format(
                        request_dict["cmte_id"]
                    )
                )
    except Exception as e:
        raise Exception(
            """The committee_master_get function is
            throwing an error: """
            + str(e)
        )
...
def get_cmte_type(cmte_id):
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                """SELECT cmte_type_category FROM public.committee_master
                WHERE cmte_id=%s""",
                [cmte_id],
            )
            ...
            if cmte_type_tuple:
                ...
                raise Exception(
                    """The cmte_id: {} does not exist in committee master
                    table.""".format(
                        cmte_id
                    )
                )
    ...
    try:
        if report_id not in [None, "", " ", "null"]:
            ...
            with connection.cursor() as cursor:
                ...
                if cursor.rowcount == 0:
                    raise Exception(
                        """The report Id: {0} for committee Id: {1}
                        doesnot exist or is deleted.""".format(
                            report_id, cmte_id
                        )
                    )
                else:
                    ...
                    if report_status not in ["Saved", "", None, " "] and submit_flag:
                        raise Exception(
                            """The report Id: {0} for committee Id: {1}
                            is already submitted.""".format(
                                report_id, cmte_id
                            )
                        )
        else:
            raise Exception(
                """Report Id cannot be none or blank.
                Kindly check. Input received: """
                + report_id
            )
    except Exception as e:
        raise e
...
def report_post(request):
    try:
        ...
        raise Exception("The report_post function is throwing an error: " + str(e))
...
def report_put(request_dict):
    try:
        ...
        raise Exception("The report_put function is throwing an error: " + str(e))
...
def report_delete(report_dict):
    try:
        ...
        raise Exception("The report_delete function is throwing an error: " + str(e))
...
def report_remove(report_dict):
    try:
        ...
        raise Exception("The report_remove function is throwing an error: " + str(e))
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if output_dict:
                ...
                raise Exception(
                    """The report Id: {} does not exist in reports
                    table for committeeId: {}""".format(
                        request_dict["report_id"], request_dict["cmte_id"]
                    )
                )
    except Exception as e:
...
def get_candidate_details(request_dict):
    try:
        ...
        for i in range(1, 6):
            ...
            if candidate_id:
                candidate_dict = get_sql_candidate(
                    candidate_id, request_dict["cmte_id"]
                )
                candidate_dict["contribution_date"] = request_dict.get(
                    column_name[:-2] + "con"
                )
...
def step4_reports_put(request):
    try:
        report_dict = {}
        if "additionalEmail1" in request.data and request.data[
            "additionalEmail1"
        ] not in [None, "", " ", "null"]:
            report_dict["additional_email_1"] = request.data["additionalEmail1"]
        else:
            report_dict["additional_email_1"] = None
        if "additionalEmail2" in request.data and request.data[
            "additionalEmail2"
        ] not in [None, "", " ", "null"]:
...
def submit_f1m_report(request):
    try:
        ...
        raise Exception("The submit_report function is throwing an error: " + str(e))
...
def f1M_clear(cmte_id, report_id, request_list):
    try:
        if request_list:
            sql_dict = {"cmte_id": cmte_id, "report_id": report_id}
        ...
        raise Exception("The f1M_clear function is throwing an error: " + str(e))
...
def get_sql_f1m(request_dict, complete=False):
    try:
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                """SELECT json_agg(t) FROM ({}) AS t""".format(sql), value_list
            )
            ...
            if output_dict:
                ...
                raise Exception(
                    """This report Id: {} for committee Id: {}
                    does not exist in forms_1m table""".format(
                        request_dict["report_id"], request_dict["cmte_id"]
                    )
                )
    except Exception as e:
        raise Exception("The get_sql_f1m function is throwing an error: " + str(e))
...
def validate_before_submit(request_dict):
    try:
        if request_dict["establishmentType"] == "A":
            ...
            raise Exception(
                """The establishment status is not correctly defined.
...
def f1m_post(request_dict):
    try:
        ...
        raise Exception("The f1m_post function is throwing an error: " + str(e))
...
def f1m_put(request_dict):
    try:
        ...
        raise Exception("The f1m_put function is throwing an error: " + str(e))
...
def f1m_delete(request_dict):
    try:
        ...
        raise Exception("The f1m_delete function is throwing an error: " + str(e))
...
def f1m_remove(request_dict):
    try:
        ...
        raise Exception("The f1m_remove function is throwing an error: " + str(e))
    ...
    ************************************ POST Call - form 1M ******************************************
    ...
    if request.method == "POST":
        try:
            noneCheckMissingParameters(
                ["step"],
                checking_dict=request.query_params,
                value_dict=request.query_params,
                function_name="form1M-POST",
            )
            ...
            if step == "saveAffiliation":
                try:
                    ...
                    noneCheckMissingParameters(
                        ["committee_id", "affiliation_date", "committeeType"],
                        checking_dict=request.data,
                        value_dict=request.data,
                        function_name="form1M-POST: step-2 Affiliation",
                    )
                    ...
                    if "report_id" in request_dict:
                        ...
                        check_clear_establishment_status(
                            cmte_id, request_dict["report_id"], "Q"
                        )
            ...
            # by qualification step2 POST - skipping candidates
                ...
                try:
                    ...
                    if "report_id" in request_dict:
                        ...
                        previous_f1m_dict = get_sql_f1m(request_dict)
                        check_clear_establishment_status(
                            cmte_id, request_dict["report_id"], "A"
                        )
                        f1m_flag, output_dict = f1m_put(request_dict)
                    else:
                        report_flag, request_dict["report_id"] = report_post(request)
            ...
            # by qualification step2 POST
                ...
                try:
                    ...
                    if request.data["beneficiary_cand_entity_id"].startswith("FEC"):
                        new_entity_id = clone_fec_entity(
                            cmte_id, "CAN", request.data["beneficiary_cand_entity_id"]
                        )
                        logger.debug("NEW candidate entity_id: " + new_entity_id)
                    candidate_number = check_candidate_number(
                        request.data["candidate_number"]
                    )
                    request_dict = f1m_sql_dict(cmte_id, step, request.data)
                    column_name = "can" + candidate_number + "_id"
                    request_dict[column_name] = request.data["candidate_id"]
                    request_dict[column_name[:-2] + "con"] = request.data[
                        "contribution_date"
                    ]
                    ...
                    if "report_id" in request_dict:
                        ...
                        request_dict["committee_type"] = request.data.get(
                            "committeeType"
                        )
                        report_flag, request_dict["report_id"] = report_post(request)
                        f1m_flag, output_dict = f1m_post(request_dict)
                        ...
                        raise Exception(
                            "reportId parameter is mandatory for this step."
                        )
                except Exception as e:
                    json_result = {"message": str(e)}
                    return JsonResponse(
            ...
            # both step4 POST
                ...
                raise Exception(
                    """Kindly provide a valid value for 'step' parameter.
                ...
                # by qualification step2 PUT
                    ...
                    if candidate_number == "1":
                        if "committeeType" in request.data:
                            request_dict["committee_type"] = (
                                request.data.get("committeeType")
                                if request.data["committeeType"]
                                else None
                            )
                        check_clear_establishment_status(
                            cmte_id, request_dict["report_id"], "A"
    ...
    if request.method == "GET":
        try:
            ...
            noneCheckMissingParameters(
                ...
                checking_dict=request.query_params,
                value_dict=request.query_params,
                function_name="form1M-GET",
            )
            request_dict = f1m_sql_dict(cmte_id, step, request.query_params)
            check_report_id_status(
                cmte_id,
                request.query_params["reportId"],
                delete_flag=False,
                submit_flag=False,
            )
            output_dict = get_sql_f1m(request_dict, True)
...
def noneCheckTrueorFalse(parameter, check_dict):
    try:
        if parameter in check_dict and check_dict[parameter] not in [
            None,
            "",
            " ",
            "null",
        ...
        logger.debug(e)
        return Response(
            "The noneCheckTrueorFalse is throwing an error: " + str(e),
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    try:
        ...
        if type == "committee":
            if noneCheckTrueorFalse("committee_id", request.query_params):
                ...
                raise Exception(
                    """For committee type, only committee_id,
            ...
            if noneCheckTrueorFalse("candidate_id", request.query_params):
                ...
                raise Exception(
                    """For candidate type, only candidate_id, cand_last_name,
            ...
            sql = """SELECT cand_id AS "candidate_id", cand_last_name, cand_first_name,
                ...
                ORDER BY cand_last_name ASC, cand_first_name ASC""".format(
                param_string
            )
        elif type == "treasurer":
            if noneCheckTrueorFalse("treasurer", request.query_params):
                ...
                raise Exception(
                    """For treasurer type, only treasurer is
                    valid parameters"""
                )
            ...
            raise Exception(
                """type parameter can have only these values : 'committee',
                'candidate', 'treasurer'"""
            )
        with connection.cursor() as cursor:
        ...
        logger.debug(e)
        return Response(
            "The get_details API is throwing an error: " + str(e),
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def get_original_registration_date(request):
    try:
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                """SELECT json_agg(t) FROM ({}) AS t""".format(sql), [cmte_id, cmte_id]
            )
            ...
            if output_dict:
                ...
                raise Exception(
                    """This committee doesnot have entries in form_1 table.
                    Kindly check with DB admin. committeeId: {}""".format(
                        cmte_id
                    )
                )
        return JsonResponse(output_dict, status=status.HTTP_200_OK, safe=False)
        ...
        logger.debug(e)
        return Response(
            "The get_original_registration API is throwing an error: " + str(e),
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if output_list:
                ...
                raise Exception(
                    """The reportId: {} does not exist in form 1m table
                    for committeeId: {}""".format(
                        request.data["reportId"], cmte_id
                    )
                )
        ...
        logger.debug(e)
        return Response(
            "The get_committee_met_req_date API is throwing an error: " + str(e),
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def delete_candidate_f1m(request):
    try:
        ...
        try:
            ...
            candidate_number = check_candidate_number(
                request.query_params["candidate_number"]
            )
