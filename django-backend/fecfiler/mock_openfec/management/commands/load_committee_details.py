...
class Command(BaseCommand):
    help = "Load mock committee details into redis"
    ...
    def handle(self, *args, **options):
        if MOCK_OPENFEC_REDIS_URL:
            ...
            if not options.get("s3"):
                path = os.path.join(
                    BASE_DIR, "mock_openfec/management/commands/committee_details.json"
                )
                with open(path) as file:
                    committee_details = file.read()
            else:
                s3_object = S3_SESSION.Object(
                    AWS_STORAGE_BUCKET_NAME, "mock_committee_details.json"
                )
                file = s3_object.get()["Body"]
                committee_details = file.read()
            redis_instance.set(COMMITTEE_DETAILS_REDIS_KEY, committee_details)

            self.stdout.write(self.style.SUCCESS("Successfully loaded committee details"))
