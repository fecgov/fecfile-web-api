...
def submit_to_fec(dot_fec_id, upload_submission_record_id, e_filing_password):
    ...
    logger.info(f"Running submission task for: {dot_fec_record.report_id}")
    ...
    logger.info(f"uploading {file_name} to FEC {file_bytes}\n{submission_json}")
    upload_response_string = test_fec_upload_client.service.upload(
        submission_json, file_bytes
    )
    logger.info(f"upload_response: {upload_response_string}")
    submission_record = update_submission_status(
        upload_submission_record_id, json.loads(upload_response_string)
    )

    while submission_record.fec_status not in ["ACCEPTED", "REJECTED"]:
        time.sleep(2)
        status_response_string = test_fec_upload_client.service.status(
            submission_record.fec_submission_id
        )
...
def update_submission_status(upload_submission_record_id, response_json):
    ...
    upload_submission_record.fec_submission_id = response_json["submission_id"]
    upload_submission_record.fec_status = response_json["status"]
    upload_submission_record.fec_message = response_json["message"]
