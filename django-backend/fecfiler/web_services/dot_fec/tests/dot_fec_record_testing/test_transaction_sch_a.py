...
class DotFECSchARecordsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.contact_com = create_test_committee_contact(
            "com name", "C01010101", self.committee.id, {
                "street_1": "45 Test Dr",
                "street_2": "Apt. A",
                "city": "Testopolis",
                "state": "CA",
        ...
        self.contact_can = create_test_candidate_contact(
            "Last", "First", self.committee.id, "S1AL00101", "S", "AL", "01", {
                "middle_name": "Middle",
                "prefix": "Mr.",
                "suffix": "III",
                "street_1": "1234 Test Ln",
        ...
        self.transaction_com_row = serialize_instance(
            "SchA",
            self.transaction_com
        ).split(FS_STR)
        self.transaction_org_row = serialize_instance(
            "SchA",
            self.transaction_org
        ).split(FS_STR)
        self.transaction_ind_row = serialize_instance(
            "SchA",
            self.transaction_ind
        ).split(FS_STR)
        self.transaction_agg_row = serialize_instance(
            "SchA",
            self.transaction_agg
        ).split(FS_STR)
        self.transaction_donor_row = serialize_instance(
            "SchA",
            self.transaction_donor
        ).split(FS_STR)
    ...
    def test_transaction_id(self):
        self.assertEqual(
            self.transaction_ind_row[2],
            self.transaction_ind.transaction_id
        )
        self.assertEqual(
            self.transaction_org_row[2],
            self.transaction_org.transaction_id
        )
        self.assertEqual(
            self.transaction_com_row[2],
            self.transaction_com.transaction_id
        )

    def test_back_reference_fields(self):
        self.assertEqual(
            self.transaction_ind_row[3],
            self.transaction_org.transaction_id
        )
        self.assertEqual(
            self.transaction_org_row[3],
            self.transaction_com.transaction_id
        )
    ...
    def test_donor_fields(self):
        ...
        self.assertEqual(
            self.transaction_donor_row[33],
            self.contact_can.candidate_office
        )
        self.assertEqual(
            self.transaction_donor_row[34],
            self.contact_can.candidate_state
        )
        self.assertEqual(
            self.transaction_donor_row[35],
            self.contact_can.candidate_district
        )
