...
class OpenfecViewSetTest(TestCase):
    ...
    def test_search_committee_valid_committee_id(self):
        request = self.factory.get("/api/v1/openfec/query-filings?query=C99999999&form_type=F1")
        request.user = self.user
        with patch("fecfiler.openfec.views.requests") as mock_requests:
            mock_requests.get.return_value = mock_response = Mock()
            mock_response.status_code = 200
    ...
    def test_search_committee_invalid_committee_id(self):
        request = self.factory.get("/api/v1/openfec/query-filings?query=NOTVALID&form_type=F1")
        request.user = self.user
        with patch("fecfiler.openfec.views.requests") as mock_requests:
            mock_requests.get.return_value = mock_response = Mock()
            mock_response.status_code = 200
            ...
            self.assertEqual(response.data, "NOTVALID is not part of a valid committee id")
