...
def update_user_selection(
    entity_id, user_selected_val, option_selected, cmte_id, file_name
):
    ...
    try:
        with connection.cursor() as cursor:
            _sql = (
                """UPDATE public.entity_import_temp SET """
                + selected_field_val
                + """= %s, """
                + other_option_1
                + """ = %s, """
                + other_option_2
                + """ = %s WHERE entity_id = %s AND cmte_id = %s AND file_name = %s"""
            )
            _v = (user_selected_val, "", "", entity_id, cmte_id, file_name)
            ...
            if request.method == "POST":
                ...
                if user_option.lower() == "add":
                    ...
                    update_user_selection(
                        file_entity_id, "true", "add", cmte_id, file_name
                    )
                elif user_option.lower() == "update":
                    update_user_selection(
                        file_entity_id, db_entity_id, "update", cmte_id, file_name
                    )
                elif user_option.lower() == "exsisting":
                    update_user_selection(
                        file_entity_id, db_entity_id, "exsisting", cmte_id, file_name
                    )

                all_selected = check_if_all_options_selected(cmte_id, file_name)

                return JsonResponse(
                    {"msg": "Success", "allDone": all_selected},
...
def get_update_contact(cmte_id, file_name, choice):
    ...
    try:
        with connection.cursor() as cursor:
            query_string = (
                """SELECT entity_id, cmte_id, transaction_id, file_name, entity_type, street_1, street_2, city, state, """
                + field
                + """, zip_code,employer,occupation,entity_name,last_name,first_name, middle_name, preffix, suffix
                                                FROM public.entity_import_temp WHERE cmte_id = %s AND file_name = %s AND """
                + field
                + """ not in ('', ' ')"""
            )
            cursor.execute(
...
def create_entity_log_model(old_update_list, username):
    try:
        ...
        logger.debug(e)
        raise NoOPError(
            "Error occurred while saving bulk contacts history log to database.", e
        )
...
def create_temp_transaction_association_model(total_list, table_name):
    try:
        ...
        with connection.cursor() as cursor:
            all_contact = [
                {**contact, "table_name": table_name} for contact in total_list
            ]
            ...
            psycopg2.extras.execute_batch(
                ...
                """
                        INSERT INTO public."""
                + table_name
                + """ (entity_id, cmte_id, transaction_id, file_name) VALUES (
                            %(entity_id)s,
                            %(cmte_id)s,
                            %(transaction_id)s,
                            %(file_name)s
    ...
    try:
        ...
        try:
            if request.method == "POST":
                ...
                if (
                    exsist_list is not None
                    and len(exsist_list) > 0
                    and transaction_included
                ):

                    for exsist_contact in exsist_list:
                        old_exsist_entity_list.append(
                            {
                                "entity_id": exsist_contact["exsisting_db"],
                ...
                if transaction_included:
                    ...
                    for contact in contact_updated_list:
                        contact_new_update_list.append(
                            {
                                "entity_id": contact["update_db"],
                ...
                return JsonResponse(
                    {"msg": "Successfully Merged Contacts"},
