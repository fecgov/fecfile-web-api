...
class Form3ViewSet(ReportViewSet):
    """
    This viewset automatically provides `list`, `create`, `retrieve`,
    `update` and `destroy` actions.
    """

    queryset = Report.objects.select_related("form_3").filter(
        report_type=ReportType.F3.value
    )
    ...
    def get_final_report(self, request):
        ...
        return Response(Form3Serializer(final_report).data)
