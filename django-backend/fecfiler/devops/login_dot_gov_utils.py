...
def stage_login_dot_gov_pk(
    ...
    rsa_pk_creds_bytes = rsa_pk_to_creds_bytes(rsa_pk)
...
def rsa_pk_to_creds_bytes(rsa_pk: rsa.RSAPrivateKey):
    return b"\n".join(rsa_pk_to_bytes(rsa_pk).splitlines())


def stage_login_dot_gov_cert(x509_cert: Certificate):
    x509_cert_bytes = x509_cert_to_bytes(x509_cert)
    filename = f"public_{datetime.utcnow().strftime('%Y%m%d%H%M%S')}.crt"
