...
def check_mandatory_fields_SE(data):
    ...
    try:
        ...
        if errors:
            raise Exception(
                'The following mandatory fields are required in order to save data to schedE table: {}'.format(','.join(errors)))
    except:
        raise
...
def put_sql_schedE(data):
    ...
    _v = (
            ...
            data.get('dissemination_date', None),
            data.get('expenditure_amount', None),
            data.get('disbursement_date', None),
            data.get('calendar_ytd_amount', None),
            data.get('purpose', ''),
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if schedE_list is None:
                raise NoOPError('No sched_E transaction found for report_id {} and cmte_id: {}'.format(
                    report_id, cmte_id))
            merged_list = []
...
def get_list_schedE(report_id, cmte_id, transaction_id):
    try:
        with connection.cursor() as cursor:
            ...
            schedE_list = cursor.fetchone()[0]
            if schedE_list is None:
                raise NoOPError('No sched_e transaction found for transaction_id {}'.format(
                    transaction_id))
            merged_list = []
            for dictE in schedE_list:
                merged_list.append(dictE)
...
def delete_schedE(data):
    ...
    try:
        ...
        delete_sql_schedE(data.get('cmte_id'), data.get(
...
def schedE(request):
    ...
    if request.method == 'POST':
        try:
            ...
            return Response("The schedE API - POST is throwing an exception: "
        ...
        try:
            ...
            logger.debug(e)
            return Response("The schedE API - GET is throwing an error: " + str(e), status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response("The schedE API - DELETE is throwing an error: " + str(e), status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            logger.debug(e)
            return Response("The schedE API - PUT is throwing an error: " + str(e), status=status.HTTP_400_BAD_REQUEST)
