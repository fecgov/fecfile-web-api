...
class TransactionSerializerBaseTestCase(TestCase):
    ...
    def test_to_representation_includes_reports(self):
        report = create_form3x(self.committee, "2024-04-01", "2024-04-30", {})
        transaction = create_schedule_a(
            "INDIVIDUAL_RECEIPT", self.committee, None, "2024-04-12", 50, report=report
        )

        serializer = TransactionSerializer(context={"request": self.mock_request})
        representation = serializer.to_representation(transaction)

        self.assertIn("report_ids", representation)
        self.assertEqual(representation["report_ids"], [report.id])
        self.assertEqual(len(representation["reports"]), 1)
        self.assertEqual(representation["reports"][0]["id"], report.id)

    def test_to_representation_includes_reports_for_nested_reatt_redes(self):
        report = create_form3x(self.committee, "2024-01-01", "2024-01-31", {})
        original = create_schedule_a(
            "INDIVIDUAL_RECEIPT", self.committee, None, "2024-01-10", 100, report=report
        )
        original.schedule_a.reattribution_redesignation_tag = REATTRIBUTED
        original.schedule_a.save()

        reattribution_to = create_schedule_a(
            "INDIVIDUAL_RECEIPT", self.committee, None, "2024-01-11", 25, report=report
        )
        reattribution_to.reatt_redes = original
        ...
        serializer = TransactionSerializer(context={"request": self.mock_request})
        representation = serializer.to_representation(reattribution_to)

        self.assertIn("reatt_redes", representation)
        self.assertEqual(representation["reatt_redes"]["report_ids"], [report.id])
        self.assertEqual(len(representation["reatt_redes"]["reports"]), 1)
        self.assertEqual(representation["reatt_redes"]["reports"][0]["id"], report.id)

    def test_to_representation_no_depth_with_add_reports_includes_report_ids(self):
        report = create_form3x(self.committee, "2024-04-01", "2024-04-30", {})
        transaction = create_schedule_a(
            "INDIVIDUAL_RECEIPT", self.committee, None, "2024-04-12", 50, report=report
        )
        ...
        serializer = TransactionSerializer(
            context={"request": self.mock_request, "no_depth": True, "add_reports": True},
        )
        representation = serializer.to_representation(transaction)
