...
class SchATransactionsViewTest(TestCase):
    ...
    def setUp(self):
        self.f3x = 1
        self.f3x_2=2
        self.f3x_transaction_count = len(SchATransaction.objects.filter(report_id=self.f3x))
        self.f3x_2_transaction_count= len(SchATransaction.objects.filter(report_id=self.f3x_2))
    ...
    def test_url_requires_no_report_id(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(parsed_response["count"], self.f3x_transaction_count+self.f3x_2_transaction_count)
    ...
    def test_only_from_one_report(self):
        response_f3x = self.client.get(f'/api/v1/sch-a-transactions/?report_id={self.f3x}').json()
        response_f3x_2=self.client.get(f'/api/v1/sch-a-transactions/?report_id={self.f3x_2}').json()
        self.assertNotEqual(response_f3x["count"], response_f3x_2["count"])
