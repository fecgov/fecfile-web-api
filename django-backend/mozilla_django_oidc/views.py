...
class OIDCAuthenticationCallbackView(View):
    ...
    def login_success(self):
        # If the user hasn't changed (because this is a session refresh
        # instead of a normal login), don't call login.
    ...
    def get(self, request):
        ...
        if request.GET.get("error"):
            ...
            # State, Nonce and PKCE Code Verifier are stored
            # in the session "oidc_states" dictionary.
            # State is the key, the value is a dictionary
            # with the Nonce in the "nonce" field, and
            ...
            # Get the nonce and optional code verifier
            # from the dictionary for further processing
            ...
            request.session.save()
            # Reset the session. This forces the session to get reloaded
            # from the database after
            # fetching the token from the OpenID connect provider.
            # Without this step we would overwrite items that are
            # being added/removed from the session in parallel browser tabs.
