...
def default_username_algo(unique_identifier, claims=None):
    ...
    :arg str/unicode unique_identifier: the unique_identifier
    to use to generate a username
...
class OIDCAuthenticationBackend(ModelBackend):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            ...
            msg = "{} alg requires OIDC_RP_IDP_SIGN_KEY or OIDC_OP_JWKS_ENDPOINT to be configured."  # noqa
    ...
    def retrieve_matching_jwk(self, token):
        ...
        for jwk in jwks["keys"]:
            if import_from_settings("OIDC_VERIFY_KID", True) and jwk["kid"] != smart_str(
                header.kid
            ):
                continue
    ...
    def get_token(self, payload):
        ...
        if self.get_settings("OIDC_TOKEN_USE_BASIC_AUTH", False):
            # When Basic auth is defined, create the Auth Header and
