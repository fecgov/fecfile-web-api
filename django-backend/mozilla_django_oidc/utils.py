...
def base64_url_decode(string_like_obj):
    ...
    https://datatracker.ietf.org/doc/html/rfc7636#appendix-A
    This function is not used by the OpenID client;
    it's just for testing PKCE related functions.
    """
    s = string_like_obj
...
def add_state_and_verifier_and_nonce_to_session(
    ...
    """
    Stores the `state` and `nonce` parameters and an optional `code_verifier`
    (for PKCE) in a session dictionary which maps
    `state` -> {nonce, code_verifier}.
    Each entry includes the time when it was added.
    The dictionary can contain multiple state -> {nonce, code_verifier} mappings
    to allow parallel logins with multiple browser sessions.
    ...
    # The "oidc_states" dictionary uses the state as key and
    # as value a dictionary with "nonce" and "added_on".
    # "added_on" contains the time when the state was added to the session.
    ...
    # remove the oldest state by finding out
