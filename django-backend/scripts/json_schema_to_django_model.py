...
def get_required_string(
    key_name, required_fields, field_type="string", is_pk_field=False
):
    """
...
def parse_model(json_model):  # noqa
    ...
    if json_model["type"] != "object":
        print(
            "Model type has to be object to convert to model, got {}".format(
                json_model["type"]
            )
        )
    ...
    for key_name, key_attributes in json_model["properties"].items():
        ...
        if key_attributes["type"] == "null":
            print(
                "ERROR: Unsupported type null, skipping for field {}".format(key_name)
            )
        ...
        # If required field
        required_str = get_required_string(
            key_name, required_fields, key_attributes["type"], is_pk_field
        )
        field_str = ""
        ...
        if key_attributes["type"] == "string" and "enum" in key_attributes:
            ...
            field_str = (
                "    {} = models.CharField(choices={}, max_length={}, "
                "default='{}', {})\n"
                "".format(
                    key_name,
                    choices,
                    max_length,
                    key_attributes["enum"][0],
                    required_str,
                )
            )
        ...
        # Date time field
            ...
            and key_attributes.get("format") == "date-time"
        ):
            auto_now_add = False
            editable = True
            ...
            field_str = (
                "    {} = models.DateTimeField(auto_now_add={}, editable={}, {})\n"
            ...
            max_length = key_attributes.get("maxLength")
            max_length_str = (
                ", max_length={}".format(max_length) if max_length is not None else ""
            )
            min_length = key_attributes.get("minLength")
            min_length_str = (
                ", min_length={}".format(min_length) if min_length is not None else ""
            )
            field_str = "    {} = models.TextField({}{}{})\n".format(
                key_name, required_str, max_length_str, min_length_str
            )
