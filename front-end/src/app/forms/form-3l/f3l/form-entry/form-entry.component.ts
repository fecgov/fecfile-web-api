  ...
  setupAdditionalFormControls() {
    if(this.reportDetails && this.frmIndividualReceipt && this.frmIndividualReceipt.controls){
      ...
      if(!this.reportDetails.cvgStartDate && !this.reportDetails.cvgEndDate){
        this.frmIndividualReceipt.controls['quarterly_monthly_covered_amount'].disable();
      }else{
        this.frmIndividualReceipt.controls['quarterly_monthly_covered_amount'].enable();
      }

      //no semi-annual dates scenario
      if(!this.reportDetails.semiAnnualStartDate && !this.reportDetails.semiAnnualEndDate){
        this.frmIndividualReceipt.controls['semi_annual_covered_amount'].disable();
      }else{
        this.frmIndividualReceipt.controls['semi_annual_covered_amount'].enable();
      }
  ...
  public handleOnBlurEvent($event: any, col: any) {
    ...
    if (this.isFieldName(col.name, 'contribution_amount') || this.isFieldName(col.name, 'expenditure_amount') ||
      this.isFieldName(col.name, 'quarterly_monthly_covered_amount') || this.isFieldName(col.name, 'semi_annual_covered_amount')
        ...
        if(this.isFieldName(col.name, 'quarterly_monthly_covered_amount')){
        ...
        else if(this.isFieldName(col.name, 'semi_annual_covered_amount')){
          if(amountEntered > this.maximumThresholdAmount){
