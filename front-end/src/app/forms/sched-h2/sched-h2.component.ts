...
export class SchedH2Component extends AbstractSchedule implements OnInit, OnDestroy, OnChanges {
  ...
  public handleOnFedBlurEvent(e) {
    if (e.target.value <= 100) {
      ...
      this.schedH2.patchValue(
        { federal_percent: this._decPipe.transform(Number(e.target.value), '.2-2') },
        { onlySelf: true }
      );
    } else {
      this.schedH2.patchValue({ non_federal_percent: 0 }, { onlySelf: true });
  ...
  public handleOnNonFedBlurEvent(e) {
    if (e.target.value <= 100) {
      ...
      this.schedH2.patchValue(
        { non_federal_percent: this._decPipe.transform(e.target.value, '.2-2') },
        { onlySelf: true }
      );
    } else {
      this.schedH2.patchValue({ federal_percent: 0 }, { onlySelf: true });
  ...
  public editH2(item: any) {
    ...
    this.schedH2.patchValue(
      { select_activity_function: item.event_type === 'fundraising' ? 'f' : 'd' },
      { onlySelf: true }
    );
    this.schedH2.patchValue({ ratio_code: item.ratio_code }, { onlySelf: true });
    this.schedH2.patchValue(
      { federal_percent: this._decPipe.transform(item.federal_percent * 100, '.2-2') },
      { onlySelf: true }
    );
    this.schedH2.patchValue(
      { non_federal_percent: this._decPipe.transform(item.non_federal_percent * 100, '.2-2') },
      { onlySelf: true }
    );
