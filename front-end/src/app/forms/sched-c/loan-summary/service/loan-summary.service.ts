...
export enum LoanSummaryActions {
  add = 'add',
  edit = 'edit'
}
...
export class LoanSummarysService {
  ...
  private createMockTrx() {
    ...
    */
   public trashOrRestoreContacts(action: string, contacts: Array<LoanModel>) {
   ...
   *
   * @param      {string}           formType  The form type
   * @param      {LoanSummaryActions}  scheduleAction  The type of action to save (add, edit)
   */
  public saveContact(scheduleAction: LoanSummaryActions): Observable<any> {
    ...
    if (scheduleAction === LoanSummaryActions.add) {
      return this._http
    ...
    } else if (scheduleAction === LoanSummaryActions.edit) {
      return this._http
