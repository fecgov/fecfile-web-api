...
export class TransactionsService {
  ...
  public mockApplyFilters(response: any, filters: TransactionFilterModel, view: ActiveView) {
    ...
    if (view === ActiveView.recycleBin) {
      if (filters.filterFiledDateFrom && filters.filterFiledDateTo) {
        isFilter = true;
        const filterDeletedDateFromDate = new Date(filters.filterFiledDateFrom + ' EST');
        const filterDeletedDateToDate = new Date(filters.filterFiledDateTo + ' EST');
  ...
  public getStates(formType: string, transactionType: string): Observable<any> {
    ...
    console.log(" Accesssing getStates ...!");
    return this._http
        .get(
          `${environment.apiUrl}${url}`,
          {
